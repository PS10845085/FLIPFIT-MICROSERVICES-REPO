...
public class GymFlipFitAdminServiceImpl implements GymFlipFitAdminService {
	...
	public ResponseEntity<GymFlipFitAdmin> findCustomerById(Long id) {
		GymFlipFitAdmin data = adminRepo.findById(id).orElseThrow(() -> new RuntimeException("User ID not found"));
	...
	public List<GymFlipFitCustomer> findAllCustomers() {
		List<com.lti.flipfit.entity.GymFlipFitCustomer> allCustomerData =	custRepo.findAll();
		return (List<GymFlipFitCustomer>) ResponseEntity.status(200).body(allCustomerData);
	}
